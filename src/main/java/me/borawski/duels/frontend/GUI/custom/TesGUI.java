...
package me.borawski.duels.frontend.GUI.custom;

import me.borawski.duels.Duels;
import me.borawski.duels.backend.queue.Queue;
import me.borawski.duels.frontend.GUI.CustomIS;
import me.borawski.duels.frontend.GUI.ItemGUI;
import me.borawski.duels.frontend.GUI.MenuItem;
                ...
                if (gui[x][y] == 1) {
                    if (q.size()-1 >= p) {
                        ...
                        set(i, new MenuItem(new CustomIS().setMaterial(q.get(p).setItem(q.get(p).getType().get(0)).getItem().getType()).setData((short) 1).setName(("&c&l" + q.get(p).getName()).replace("&", ChatColor.COLOR_CHAR + ""))
                                ...
                                .addLore(ChatColor.GRAY + "" + ChatColor.STRIKETHROUGH + "-------------------------------").addLore(ChatColor.RED + "" + ChatColor.BOLD + "* " + ChatColor.RESET
                                        + "" + ChatColor.GRAY + "Random Arena").addLore(ChatColor.RED + "" + ChatColor.BOLD + "* " + ChatColor.RESET
                                        + "" + ChatColor.GRAY + "Expected wait: " + ChatColor.RED + ((q.get(p).getList().size() % 2 == 0) ? q.get(p).getList().size()/2 + "s" : "Waiting for another player"))
